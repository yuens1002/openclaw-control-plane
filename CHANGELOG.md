...
- fix(webhook): document and harden the actual production `/hooks/agent` dispatch target
  - Confirmed directly against the live OpenClaw gateway source that the literal `/hooks/agent` endpoint hard-requires a `message` field and cannot opt out of the untrusted-external-content wrapper — this wrapper's metadata-only forward body was never going to satisfy either, so `resolveAgentHookUrl()`'s bare default is not a usable production target. Production must point `OPENCLAW_AGENT_HOOK_URL` at an OpenClaw `hooks.mappings`-configured subpath instead, where a `messageTemplate` + `allowUnsafeExternalContent: true` are declared in the consuming instance's own config. Documented in `forwardToAgentHook`'s docstring and `docs/plans/github-webhook-agent-dispatch/plan.md`; no behavior change to this wrapper's own forwarded fields.
  - Added `idempotencyKey` (the GitHub delivery id) to the forwarded body so the gateway's own replay cache can return a cached result on an identical-id replay after a prior successful dispatch, independent of this wrapper's own dedup store.
  - Fixed AC-SEC-2's stale hard-coded test count in `docs/plans/github-webhook-agent-dispatch/ACs.md`.
  - `/ocr-review` pass (two Fable-5 bundles) found and fixed: README's dispatch section still described the pre-fix body shape and presented `/hooks/agent` as a normally-usable default; a 400 against the un-overridden default logged identically to a transient failure (added a diagnostic hint); the new docstring's "dedupe a retried delivery" wording was in tension with this wrapper's own failed-forward dedup-key release (reworded to state precisely which replay case each layer covers); `idempotencyKey` was asserted only via a hand-fed unit-test payload, never through the real `X-GitHub-Delivery` header path (added to both handler-level tests); the new omission test had no positive anchor.
