...
| Gap | Disposition |
| --- | --- |
| **G2** — the setup API client exposes a destructive config-reset call that deletes the target's configuration file outright (`setup-api-client.ts`'s `reset()`). Zero production callers; no gate. | Classified **Forbidden** in §2.5. Escalated as a proposal to **delete** the capability rather than gate it — unused destructive code is safer removed than fenced. Draft at `docs/plans/live-instance-operations/draft-issue-config-reset.md`. |
