...
- D6 is complete, confirmed *before* this commit: the canary's Railway service is git-connected to `main` with "Wait for CI" enabled and its native settings (dockerfilePath, watchPatterns, healthcheck, restart policy) matching D1's reference spec, verified via two prior live config read-backs and the two `SUCCESS` deployments those earlier reconnect actions triggered. D7's push-tests come next, starting with this commit: it is the deliberate docs-only negative control for AC-FN-6 and is expected to produce **zero** new canary deployments once merged — a different, still-pending outcome from the two `SUCCESS` deployments already recorded above for D6.
