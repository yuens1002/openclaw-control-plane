...
- Root `Dockerfile`'s only `COPY` instructions with a real source path in this repo (everything else uses `--from=<stage>`, which has no repo path):
  ```
  ...
  ```
  All five are files, not directories, so each becomes an exact-path watch pattern. Root `package.json`/`package-lock.json` are never copied into the wrapper image, so they need no watch entry.
...
2. Add `deploy/openclaw-railway/canary.railway.toml` — `dockerfilePath = "Dockerfile"`, `watchPatterns` derived from the five `COPY` sources above plus `/Dockerfile`, the config file itself, and `/.dockerignore`. `[deploy]` mirrors root `railway.toml` (healthcheck/restart/volume) — a real, intentional improvement, since the canary has none of that configured today. `[variables]` is omitted: the relevant variables are already set directly on the live service.
3. Add `tests/canary-watch-patterns.test.ts` (ported from control-plane's own recovered history, not the private engine repo's copy): non-empty assertion, exact-sync-with-Dockerfile drift guard, positive matches (the five sources + Dockerfile + config file), negative matches (root `railway.toml`, docs/README/CHANGELOG, root package manifests, unrelated `scripts/*`, pinned-client `.ps1` scripts, `tests/**`, `.github/**`), plus synthetic drift-detection and `--from=`/flag-stripping cases.
...
|----|-------------|------|--------------|
| D1 | `deploy/openclaw-railway/canary.railway.toml` — canary-scoped build/deploy config-as-code | build-config | `/devops` |
...
| D6 | Live Railway reconnect of the canary service (config-as-code path + git source + "Wait for CI"), human-confirmed | verification | `/devops` |
...
- **Session 2**: the canary service's live config shows the expected git source, config-as-code path, and "Wait for CI" setting after reconnect (recorded as pass/fail, no raw config or IDs); a docs-only merge produces zero canary deployments and one public-proof deployment; a watched-path merge produces exactly one canary deployment that reaches success.
...
- Session 2 cannot start until Session 1's PR is merged to `main` — Railway reads the canary's config-as-code file from the branch it connects to.
...
- Any change to the public-proof service's Railway config or behavior.
