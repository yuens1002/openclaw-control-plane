...
Status: local verification complete on `5705b166b94195998d9aee7a0fddcfffc132764d`;
...
| AC-API-001 | D2 | security | Trusted header propagation | The API forwards only the validated `x-tool-invocation-id`; identity, actor, and authorization remain server-derived. | PASS - API tests cover tool, HTTP, and malformed-header paths. | pending | pending |
...
| AC-REVIEW-001 | D5 | test-engineer | Exact-head verification | Focused/full tests, PostgreSQL, typecheck, build, audit, Docker, diff, CI, and external review pass on the merge head. | PARTIAL - local gates pass; CI and external exact-head review pending. | pending | pending |
