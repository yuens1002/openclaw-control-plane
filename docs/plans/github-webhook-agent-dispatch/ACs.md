...
| AC-FN-6 | D2 | `/devops` | `computeSessionKey` — stable per PR/issue | Code review + direct call | For two payloads on the same repository + resource number but different heads or different comment ids, `computeSessionKey` returns the identical value while `computeDedupKey` returns different values for the same two payloads — the two functions must disagree on these inputs, not just individually pass their own tests | PASS (critical AC) — the exact pair the earlier draft bug hinged on: two payloads differing only in head SHA (and separately, only in comment id) produce an IDENTICAL session key and a DIFFERENT dedup key. Independently reproduced by the verify agent, not read off the test file | PASS · trust | |
| AC-FN-7 | D2 | `/devops` | `computeDedupKey`/`computeSessionKey` — malformed payload | Code review + direct call | A payload missing the fields an event type requires (e.g. a `pull_request` payload with no `pull_request.head.sha`) returns `undefined` from both functions, never a key built from partial/undefined data | PASS — 14 malformed-payload variants constructed (missing/empty/wrong-type fields, unsupported event, null/string/proto-polluted payloads) all return `undefined` from both functions | PASS · trust | |
...
| AC-FN-13 | D3 | `/devops` | `handleGithubWebhookVerify` — end-to-end dispatch decision | Code review + integration test against the full handler | A verified, allowlisted, non-duplicate delivery results in exactly one `forwardToAgentHook` call with the session key from AC-FN-6; a verified delivery that is unenrolled, unsupported, untrusted, or a repeat of an already-forwarded dedup key results in zero forward calls; in every case the HTTP response to GitHub (200 on verified signature) is unaffected | PASS — exactly one forward on match with the correct session key; zero forwards on duplicate/unenrolled/unsupported-event/untrusted-actor/malformed-allowlist, all still 200 | PASS · trust | |
