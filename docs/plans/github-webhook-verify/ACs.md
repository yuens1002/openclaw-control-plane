...
| AC-TST-6 | D2 | `/test-engineer` | `patch-wrapper-github-webhook` — missing anchor | Test run: `npm test` | Running the patch script against a fixture with the `app.use(express.json({ limit: "1mb" }));` anchor removed exits non-zero with an occurrence-count-0 message; the fixture file is left untouched | | | |
