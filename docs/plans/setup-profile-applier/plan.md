...
- **Hard blocker before Session 1 Implement starts**: validate
  ...
  private agency/client profile repo. This repo cannot read any such private
  repo directly — the human running this plan must supply one real generated
  profile (with secret values stripped/replaced) to check field names and
  nesting against D1's schema before D1 is considered done. Field names in
  this plan are reconstructed from issue #7's prose description of one such
  repo and are not independently confirmed; D1's schema must not assume that
  repo is the only valid producer of a profile (unrecognized fields must
  pass through, per the tolerant-parsing design decision above).
